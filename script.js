...
// Génère les chips de la modale (saisons)
...
function updateConfigUI() {
  // Chips déjà gérés par generateConfigChips
...
function renderDishes() {
  ...
  filtered.forEach(dish => {
    ...
    // SECURITÉ CRITIQUE : Conversion ID en string
...
function renderMenus() {
  ...
  menus.forEach((menu, index) => { // Ajout index pour gestion ouverture
    const div = document.createElement('div');
    div.className = 'card';
    let scheduleHTML = '';
    ...
    // CORRECTION ACCORDÉON : Seulement le premier ouvert
  ...
  if (groupId) {
    ...
    database.ref(`groups/${groupId}/menus`).on('value', s => {
      ...
      if (!d) {
         ...
         all.forEach(m => {
            if(m.weekNumber) {
                if(!unique[m.weekNumber] || String(m.id).localeCompare(String(unique[m.weekNumber].id)) > 0) {
...
window.generateMenu = function() {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    schedule.push({ day, lunch, dinner, isSportDay: (menuConfig.sportDays || []).includes(day) });
  }
...
window.onload = function() {
  ...
  // Initialiser les chips pour la modale
  generateModalSeasonChips();

  setupPWA();
  
  if (groupId) {
    // Affiche l'app immédiatement si on a l'ID
    window.showMainApp();
    // Puis tente de charger les données
    initFirebaseAndListen();
  } else {
